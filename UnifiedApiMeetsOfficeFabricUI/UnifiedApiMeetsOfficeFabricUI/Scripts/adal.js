...
AuthenticationContext.prototype._renewToken = function (resource, callback) {
  ...
  this._log(resource, 'Navigate to:' + urlNavigate);
...
AuthenticationContext.prototype.handleWindowCallback = function () {
  ...
  if (this.isCallback(hash)) {
    ...
    this._log(this._getResourceFromState(requestInfo.stateResponse), "returned from redirect url");
...
AuthenticationContext.prototype._log = function (resource, message) {
  if (!this._supportsLocalStorage()) {
    return;
  }

  var logEntry = this.CONSTANTS.LOG_ENTRY + resource;

  var correlationId = this.config.correlationId;
  var timestamp = new Date().toUTCString();

  var formattedMessage = timestamp + ':' + correlationId + ': ' + message + ';';

  if (this._getItem(logEntry)) {
    this._saveItem(logEntry, this._getItem(logEntry) + formattedMessage);
  }
  else {
    this._saveItem(logEntry, formattedMessage);
  }
};
